...
Begin VB.Form frmSuppliersStmtShares 
   Caption         =   "Shares Statement"
   ClientHeight    =   3585
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   10200
   ...
   Begin VB.ComboBox ports 
      ...
      EndProperty
      Height          =   405
      ItemData        =   "frmSuppliersStmtShares.frx":0000
      Left            =   5880
      List            =   "frmSuppliersStmtShares.frx":0010
      TabIndex        =   10
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.OptionButton OptDetailedStmt 
         Appearance      =   0  'Flat
         BackColor       =   &H00C0FFFF&
         Caption         =   "Shares Statement (Use POS Printer)"
         ForeColor       =   &H80000008&
         Height          =   255
         Left            =   480
         TabIndex        =   3
   ...
   End
End
Attribute VB_Name = "frmSuppliersStmtShares"
...
If OptDetailedStmt.value = True And chkNotepad = vbUnchecked Then
                     Dim Bonusprice, amountopay, totalkgs, Totalamount, totalcash As Double
                     Bonusprice = 2
                     amountopay = 0
                     totalkgs = 0
                     Totalamount = 0
                     totalcash = 0
                      ...
                      'ttt = "LPT1" 'LPT1,LPT2....
                        ...
                        txtFile.WriteLine "" & paddress & ""
                        txtFile.WriteLine "" & town & ""
                        txtFile.WriteLine "SHARES STATEMENT AS AT " & UCase(Format(DTPStmts, "MMMM/YYYY"))
                        txtFile.WriteLine escAlignLeft
                        ''get bonus rate
...
'                        End If
                        ...
                        Set rs = oSaccoMaster.GetRecordset("SELECT    SUM(Amount) AS amt From d_supplier_deduc WHERE     (Description LIKE '%shares%') AND (SNo = '" & txtSNo & "')")
                        ...
                        'Set rs = oSaccoMaster.GetRecordset(sql)
                            If rs.EOF Or rsts.EOF Then
                                MsgBox "The supplier do not have Shares as at the Months specified", vbInformation
                        ...
                        Set rst1 = New ADODB.Recordset
                            ...
                            txtFile.WriteLine "........................................"
                            txtFile.WriteLine "Date" & vbTab & "" & vbTab & "Type" & vbTab & "Remarks" & vbTab & "" & vbTab & "AMOUNT"
                            txtFile.WriteLine "........................................"
                        
                        ''sum shares in contribution table
                        sql = "SELECT sno, transdate, amount,transdescription From d_sconribution WHERE     (transdescription LIKE '%shares%') AND (SNo = '" & txtSNo & "') order by transdate asc "
                        Set rtg = oSaccoMaster.GetRecordset(sql)
                        While Not rtg.EOF
                             txtFile.WriteLine rtg!transdate & "" & vbTab & "CASH " & vbTab & "" & rtg!transDescription & " " & vbTab & "" & Format(rtg!amount, "#,##0.0#")
                             totalcash = totalcash + rtg!amount
                          rtg.MoveNext
                        Wend
                        sql = ""
                        sql = "SELECT SNo, Date_Deduc, Description, Amount From d_supplier_deduc WHERE     (Description LIKE '%shares%') AND (SNo = '" & txtSNo & "') order by Date_Deduc asc "
                        Set rtg = oSaccoMaster.GetRecordset(sql)
                        While Not rtg.EOF
                             qnty = 0
                             GPay = 0
                                txtFile.WriteLine rtg!date_deduc & "" & vbTab & "CHECK-OFF " & vbTab & "" & rtg!description & " " & vbTab & "" & Format(rtg!amount, "#,##0.0#")
                                
                                totalkgs = totalkgs + rtg!amount
                                
                          rtg.MoveNext
                         Wend
                            Totalamount = totalkgs + totalcash
                        txtFile.WriteLine "-----------------------------------------"
                        txtFile.WriteLine "Total Cash                     : " & Format(totalcash, "#,##0.00") & ""
                        txtFile.WriteLine "Total Check-off                : " & Format(totalkgs, "#,##0.00") & ""
                        txtFile.WriteLine "Total Shares                   : " & Format(Totalamount, "#,##0.00") & ""
                        txtFile.WriteLine "-----------------------------------------"
                        txtFile.WriteLine "BANK DETAILS"
                        txtFile.WriteLine "-------------"
                        txtFile.WriteLine "Bank Name :" & rst1!bcode & ""
